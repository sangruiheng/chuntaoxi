...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 团购管理 <span class="c-gray en">&gt;</span> 开团记录 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($datalist)): $i = 0; $__LIST__ = $datalist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr  class="text-c">
                ...
                <td><?php echo ($item["need_count"]); ?></td>
                <td><?php echo ($item["uname"]); ?></td>
                <td><?php echo ($item["buy_count"]); ?></td>
                <td><?php echo (date("Y-m-d H:i:s",$item["time"])); ?></td>
                <td><?php if(($item["status"] == 1) ): ?>待支付
                <?php elseif($item["status"] == 2): ?>已支付
                <?php elseif($item["status"] == 3): ?>已取消
                <?php elseif($item["status"] == 4): ?>已过期
                <?php else: ?> 报名中<?php endif; ?></td>
                <td class="obj_1">
                    <a onclick="open_log(<?php echo ($item["id"]); ?>)">参团记录</a>
                </td>
...
<script type="text/javascript">
    function open_log(id){
        location='<?php echo U("GroupBuy/joinGroupBuyLog");?>?gid='+id;
    }
</script>
