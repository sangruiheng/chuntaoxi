...
<div class="page-container">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($productlist)): $i = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td>
                    <?php if($v["type"] == 1): ?><label style="color:green;">首页推荐</label><br /><?php endif; ?>
                    <?php if($v["is_hottype"] == 1): ?><label style="color:red;">热门推荐</label><?php endif; ?>
                </td>
                <td><?php echo ($v["sort"]); ?></td>
                <td>

                    <?php if($v["is_buff"] > 0): ?><a href="<?php echo U('Product/pro_guige');?>?pid=<?php echo ($v["id"]); ?>">
                            <?php else: ?>
                            <a href="<?php echo U('Product/set_attr');?>?pid=<?php echo ($v["id"]); ?>"><?php endif; ?><font style="color:red;">属性设置</font></a> |
                    <a href="<?php echo U('set_hottj');?>?pro_id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&name=<?php echo ($name); ?>&tuijian=<?php echo ($tuijian); ?>">热推</a> |
                    <a href="<?php echo U('set_tj');?>?pro_id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&name=<?php echo ($name); ?>&tuijian=<?php echo ($tuijian); ?>">首推</a> |
                    <a href="<?php echo U('Product/add');?>?id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&name=<?php echo ($name); ?>&tuijian=<?php echo ($tuijian); ?>">修改</a> |
