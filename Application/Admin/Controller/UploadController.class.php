...
class UploadController extends PrivilegeController{
	//************************************
	// 说明：图片上传操作方法
		...
		if($err==''){
			...
			if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
			{
				...
				else
				{
                  ...
                  if($msgType==1)
						$msg= $dmomin.$targetPath;
					else{
						$msg="{'url':'".$dmomin.$targetPath."','localname':'".$this->jsonString($localName)."','id':'1'}";
