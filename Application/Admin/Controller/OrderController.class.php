...
Vendor('RWxpay.lib.WxPay#Api');
Vendor('RWxpay.lib.WxPay#Config');
Vendor('RWxpay.lib.WxPay#JsApiPay');
Vendor('RWxpay.lib.log');
	...
	public function index(){
	    ...
	    */
		...
		foreach ($order_list as $k => &$v) {
            ...
            $order_list[$k]['u_name'] = urldecode($order_list[$k]['u_name']);
		}
		//echo $where;
...
//	}
    ...
    */
    public function back(){
        ...
        $input->SetOut_trade_no($back_info['order_sn']);            //自己的订单号
//        $input->SetTransaction_id($order['transaction_id']);    //微信官方生成的订单流水号，在支付成功中有返回
        $input->SetOut_refund_no($this->getRandom(64));            //退款单号
        $input->SetTotal_fee($back_info['price'] * 100);                        //订单总金额，单位为分
        $input->SetRefund_fee($back_info['price'] * 100);            //退款总金额 单位为分，只能为整数
